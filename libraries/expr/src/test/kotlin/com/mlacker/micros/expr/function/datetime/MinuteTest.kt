...
class MinuteTest {
    ...
    private lateinit var minute: Minute

    @BeforeEach
    fun setup() {
        every { param0.type } returns BasicType.DateTime
        minute = Minute(listOf(param0))
    }

    @Test
    fun getType() {
        Assertions.assertEquals(BasicType.Integer, minute.type)
    }
    ...
    fun eval() {
        ...
        Assertions.assertEquals(15L, minute.eval(emptyMap()))
        Assertions.assertEquals(18L, minute.eval(emptyMap()))
    }

    @Test
    fun testToString() {
        every { param0.toString() } returns "'2014-01-02 19:20:21'"
        Assertions.assertEquals("Minute('2014-01-02 19:20:21')", minute.toString())
    }
